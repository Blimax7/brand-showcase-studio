  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/80 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 md:px-10">
        ...
        <a href="#access" className="group inline-flex items-center gap-2 rounded-full border border-emerald-deep/30 px-4 py-2 text-xs uppercase tracking-[0.2em] text-emerald-deep transition hover:bg-emerald-deep hover:text-cream">
          Private access <Lock className="h-3 w-3" />
        </a>
      </div>
    </header>
  );
      ...
      <div className="relative mx-auto grid max-w-7xl grid-cols-1 gap-16 px-6 py-24 md:px-10 md:py-32 lg:grid-cols-12">
        <div className="lg:col-span-6 lg:pt-10">
          ...
          <h1 className="mt-6 font-serif text-5xl leading-[1.02] tracking-tight md:text-7xl lg:text-[92px]">
            Blimax&apos;s <span className="italic text-rose-gradient">Endorsements</span> &mdash;
          ...
          <div className="mt-10 flex flex-wrap items-center gap-4">
            <a href="#beauty" className="inline-flex items-center gap-2 rounded-full bg-gold px-6 py-3 text-sm uppercase tracking-[0.2em] text-emerald-deep transition hover:bg-rose hover:text-cream">
              See the portfolio <ArrowUpRight className="h-4 w-4" />
            </a>
            <a href="#access" className="inline-flex items-center gap-2 rounded-full border border-cream/30 px-6 py-3 text-sm uppercase tracking-[0.2em] text-cream/90 transition hover:border-rose hover:text-rose">
              Work with Blimax
            </a>
          </div>
        ...
        <div className="relative lg:col-span-6">
          <div className="relative mx-auto aspect-[4/5] w-full max-w-md overflow-hidden rounded-sm ring-1 ring-gold/30 lg:max-w-none">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-emerald-deep/70 via-transparent to-rose/10"></div>
...
function BrandMarquee() {
  ...
  return (
    <section className="border-y border-border bg-cream py-10">
      ...
      <style dangerouslySetInnerHTML={{ __html: `@keyframes marquee { from { transform: translateX(0);} to { transform: translateX(-50%);} }` }}></style>
    </section>
  );
}
...
function CategorySection({ category }: { category: Category }) {
  ...
  const anchor = category.rank === "01" ? "beauty" : category.rank === "02" ? "jewelry" : "tech";
  const dark = category.rank === "02";

  return (
    <section id={anchor} className={dark ? "bg-emerald-deep py-24 text-cream md:py-32" : "bg-background py-24 md:py-32"}>
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <div className={`grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-20 ${category.reverse ? "lg:[&>div:first-child]:order-2" : ""}`}>
          <div className="lg:col-span-6">
            <a href={`#${anchor}-brands`} className="group relative block aspect-[4/5] overflow-hidden rounded-sm" aria-label={`See brands in ${category.eyebrow}`}>
              <img
                src={category.image}
              ...
              {category.rank === "01" && (
                <div className="absolute bottom-4 left-4 flex items-center gap-2 rounded-full bg-cream/90 px-3 py-1.5 backdrop-blur">
                  ...
                  <span className="text-[10px] uppercase tracking-[0.2em] text-emerald-deep">As seen on Nykaa</span>
                </div>
              )}
            </a>
          </div>
          <div className="lg:col-span-6 lg:pt-6">
            <div className={`font-serif text-[140px] leading-none ${dark ? "text-gold/30" : "text-emerald-deep/10"}`}>
              {category.rank}
            </div>
            <h2 className={`mt-4 font-serif text-4xl leading-tight md:text-6xl ${dark ? "text-cream" : "text-emerald-deep"}`}>
              {category.title}
            </h2>
            <p className={`mt-6 max-w-xl text-lg leading-relaxed ${dark ? "text-cream/75" : "text-foreground/75"}`}>
              {category.copy}
            </p>

            {category.rank === "01" && (
              <div className="mt-10 grid grid-cols-3 gap-4">
                <img src={chanelImg} alt="Chanel" loading="lazy" width={340} height={425} className="aspect-[4/5] w-full rounded-sm object-cover ring-1 ring-emerald-deep/10" />
                <img src={sulwhasooImg} alt="Sulwhasoo" loading="lazy" width={340} height={425} className="aspect-[4/5] w-full rounded-sm object-cover ring-1 ring-emerald-deep/10" />
                <img src={nykaaImg} alt="Beauty" loading="lazy" width={340} height={425} className="aspect-[4/5] w-full rounded-sm object-cover ring-1 ring-emerald-deep/10" />
              </div>
            )}

            <ul id={`${anchor}-brands`} className="mt-10 divide-y divide-current/10 scroll-mt-24">
              {category.brands.map((b) => (
                <li key={b.name + b.product} className={dark ? "border-cream/10" : "border-emerald-deep/10"} style={{ borderTop: "1px solid currentColor", opacity: 1 }}>
                  <a href={b.href} target="_blank" rel="noopener noreferrer" className={`group grid grid-cols-12 items-baseline gap-4 py-5 transition ${dark ? "hover:text-rose" : "hover:text-rose"}`}>
                    <div className={`col-span-4 font-serif text-2xl ${dark ? "text-gold" : "text-emerald-deep"}`}>
                      {b.name}
                    </div>
                    <div className={`col-span-6 text-sm ${dark ? "text-cream/80" : "text-foreground/80"}`}>
                      {b.product}
                    </div>
                    <div className={`col-span-2 flex items-center justify-end gap-1 text-right text-[10px] uppercase tracking-[0.2em] ${dark ? "text-cream/50" : "text-muted-foreground"}`}>
                      {b.note}
            ...
            {category.rank === "01" && (
              <a href="https://www.nykaa.com" target="_blank" rel="noopener noreferrer" className="group mt-6 flex items-center justify-between rounded-sm border border-emerald-deep/15 bg-cream/50 px-6 py-5 transition hover:border-gold hover:bg-cream">
                <div>
      ...
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        ...
        <div className="mt-14 grid grid-cols-1 gap-px overflow-hidden rounded-sm bg-emerald-deep/10 sm:grid-cols-2">
          {socials.map((s) => {
            const I = s.icon;
            return (
              <a key={s.name} href={s.href} className="group flex flex-col justify-between bg-cream p-8 transition hover:bg-emerald-deep hover:text-cream">
        ...
        >
          ...
          {sent ? (
            ...
            <div className="mt-8 space-y-5">
              <div>
                <label className="text-[10px] uppercase tracking-[0.3em] text-cream/60">Your name</label>
                <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Full name" className="mt-2 w-full border-b border-cream/20 bg-transparent py-2 text-cream placeholder:text-cream/30 focus:border-gold focus:outline-none" />
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-[0.3em] text-cream/60">Email</label>
                <input type="email" required value={fromEmail} onChange={(e) => setFromEmail(e.target.value)} placeholder="you@brand.com" className="mt-2 w-full border-b border-cream/20 bg-transparent py-2 text-cream placeholder:text-cream/30 focus:border-gold focus:outline-none" />
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-[0.3em] text-cream/60">Brand / company (optional)</label>
                <input type="text" value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Company name" className="mt-2 w-full border-b border-cream/20 bg-transparent py-2 text-cream placeholder:text-cream/30 focus:border-gold focus:outline-none" />
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-[0.3em] text-cream/60">Message</label>
                <textarea required rows={4} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Tell me a little about the project…" className="mt-2 w-full resize-none border-b border-cream/20 bg-transparent py-2 text-cream placeholder:text-cream/30 focus:border-gold focus:outline-none"></textarea>
              </div>
              <button type="submit" className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-full bg-gold px-6 py-3 text-sm uppercase tracking-[0.2em] text-emerald-deep transition hover:bg-cream">
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-6 text-xs text-muted-foreground md:px-10">
        <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
          <div className="flex items-center gap-2 font-serif text-lg text-emerald-deep">
            <img src={logoImg} alt="Blimax" width={28} height={28} className="h-7 w-7 rounded-full ring-1 ring-gold/40" />
